...
- 작성일: 2026-08-30
- 최종 갱신: 2026-08-31
- 상태: **세션 0 완료 · 세션 1 PRD 사용자 검토 대기**
- 경쟁 조사: [`2026-08-30-chowon-ai-benchmark.md`](./2026-08-30-chowon-ai-benchmark.md)
- 문서 역할: 세션 0의 조사·제품 방향·PWA 타당성과 후속 결정으로 조정된 14개 세션 실행 기준을 관리한다. PRD는 세션 1에서 작성하며, 승인 전에는 프로토타입·디자인 시스템·구현으로 진행하지 않는다.
...
**후속 결정: 총 14개 세션(S0~S13)으로 진행한다.** S0~S4는 제품 정의와 구현 착수 게이트이고, S5~S13은 승인 산출물을 기준으로 한 구현·검증·독립 베타 단계다. 각 세션은 앞 세션의 승인 결과만 입력으로 사용한다.

### Phase A — 조사와 제품 정의

| 세션 | 이번 세션에서 끝낼 것 | 다음 세션 진입 조건 |
|---|---|---|
| S0 | 초원AI 조사, 제품 방향, PWA 타당성, 승인 전제, 전체 로드맵 | **완료** — 제품 방향 승인 |
| S1 | PRD, 사용자·JTBD, MVP/비범위, 지표, 권리·신뢰 게이트 | **현재** — PRD 사용자 승인 |
...
| 세션 | 이번 세션에서 끝낼 것 | 다음 세션 진입 조건 |
|---|---|---|
| S4 | 아키텍처·API·도메인·작업 분해·제한 베타 계획과 S5~S13 실행 runbook | 구현 착수 승인 |
| S5 | Next.js 모바일 웹/PWA 셸 + 일반 사용자 인증·익명 범위·동의·삭제·무기억 | 설치·오프라인 셸·인증·권한·삭제 테스트 통과 |
| S6 | 콘텐츠 권리 원장과 권리 승인 소규모 정본 적재 경로 | G1 및 승인 콘텐츠만 노출되는 게이트 통과 |

### Phase C — 핵심 사용자 경험

| 세션 | 이번 세션에서 끝낼 것 | 다음 세션 진입 조건 |
|---|---|---|
| S7 | 소비자용 RAG API, 출처 스키마, 근거 부족 중단, 평가셋 | G2 품질·안전 기준 통과 |
| S8 | 오늘·통합 검색 + 출처 있는 질문·원문 상세·용어 탐색 핵심 UX | 핵심 과업·인용·판본·권위 층 검증 통과 |
| S9 | 내 서재, 개인 메모, 허용 콘텐츠 오프라인 | 개인정보·캐시·철회 경계 검증 통과 |

### Phase D — 전달·통합·독립 베타

| 세션 | 이번 세션에서 끝낼 것 | 다음 세션 진입 조건 |
|---|---|---|
| S10 | 중립형 Web Push, 구독 관리, 조용한 시간, 앱 내 알림함 | iOS·Android 실기기 전달·해지 검증 통과 |
| S11 | 전체 통합, 보안, WCAG 2.2 AA, 성능, E2E 회귀 | G3·G4 및 제한 베타 출시 게이트 통과 |
| S12 | 독립 베타 배포, 온보딩, 운영 대시보드, 소규모 코호트 시작 | G5 권리·운영·지원 체크리스트 승인 |
| S13 | 베타 결과, 성공·가드레일 지표, 이슈 우선순위, 공식 승인 이후 결정 | 다음 버전·중단·공식화 중 하나 결정 |

S1~S3은 회당 60~120분 작업과 10~20분 검토, S4는 90~150분을 예상한다. S5~S11은 회당 120~240분 구현·검증, S12는 90~180분의 배포 준비 뒤 별도 관찰 기간, S13은 데이터 확보 후 60~120분을 예상한다.

S5~S13의 세부 프롬프트는 지금 고정하지 않는다. S4에서 승인된 PRD·프로토타입·디자인 시스템과 실제 저장소 상태를 반영해 `docs/guides/ffwpu-pwa-session-runbook.md`로 확정한다.
...
- 알림: 잠금 화면 중립형 문구가 기본
- 전체 진행: S0~S13의 14개 세션이며, 이번 작업은 S1
...
runbook에는 S5~S13 각각의 복사 가능한 실행 프롬프트를 작성해. 각 프롬프트는 선행 입력, 이번 세션 범위, 필수 산출물, 완료 조건, 검증 명령, 중단 조건을 포함하고 다음 세션 범위를 미리 구현하지 않게 해.
...
| 날짜 | 결정 주체 | 상태 | 결정 | 이유·영향 |
|---|---|---|---|---|
| 2026-08-31 | 사용자 후속 결정 | 승인 | 전체 로드맵을 S0~S15 16개에서 S0~S13 14개로 조정 | 기존 S5+S6을 새 S5(PWA 셸+인증·개인정보), 기존 S9+S10을 새 S8(오늘·검색+질문·출처·용어)로 통합한다. 권리 원장→RAG, 통합 검증→베타 배포, 베타 배포→결과 판정은 분리한다. |
| 2026-08-31 | 사용자 후속 결정 | 승인 | S5~S13 상세 구현 프롬프트를 S4에서 작성 | 승인 PRD·프로토타입·디자인 시스템과 실제 저장소 상태를 runbook 입력으로 사용한다. |

---

## 9. 세션 0 승인 기록과 세션 1 상태

### 승인 기록

| 결정 | 승인값 | 적용 범위 |
|---|---|---|
| 전체 작업 | S0~S13, 총 14개 세션 | 모든 후속 계획 |
...
위 항목은 S1 PRD 작성을 막지 않는다. 다만 S6 콘텐츠 적재와 S12 독립 베타 시작 전에는 증빙 가능한 상태로 확정해야 한다. **S1 PRD는 사용자 검토 대기이며, 승인 전 S2에 진입하지 않는다.**

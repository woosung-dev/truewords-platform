...
> 2026-09-05 경로 전환: 현재 실행·검증은 [환경 설정](environment-setup.md)·[CI](ci-cd-pipeline.md)를 우선한다. 아래 Phase 상태·오래된 테스트 개수는 당시 개발 기록이며, 현재 승인 범위는 [모노레포 M1~M4 계획](../plans/completed/2026-09-05-monorepo-migration.md)이다. Flutter/M5를 자동 착수하지 않는다.

> **"다음에 뭘 해야 하지?"** 할 때 이 문서를 보세요.
>
> **기본 원칙: Phase-Adaptive** — `/office-hours`는 Phase 시작 시에만, 일상 개발은 작업 크기에 따라 superpowers 축약/확장.
> 상세 논증: `docs/adr/23-development-process-analysis.md`
...
| `/plan-ceo-review` | CEO 관점 리뷰 | `docs/research/14-success-factors-strategy.md`, `docs/research/12-market-analysis.md` |
| `/plan-eng-review` | 엔지니어링 리뷰 | `docs/architecture/02-architecture-design.md`, `docs/architecture/05-rag-pipeline.md` |
| `/plan-design-review` | 디자인 리뷰 | `docs/research/17-design-strategy.md`, `docs/prd/16-app-feature-spec.md` |
| `/review` | 코드 리뷰 | `.ai/rules/backend.md`, `.ai/rules/rag-pipeline.md` |
| `/qa` | QA 테스트 | `docs/prd/16-app-feature-spec.md` |
| `/cso` | 보안 검토 | `docs/architecture/09-security-countermeasures.md` |
...
| 우선순위 | 항목 | 설계 문서 | 비고 |
|----------|------|-----------|------|
| **P0** | 레드팀 테스트 | `docs/architecture/09-security-countermeasures.md` | 배포 후 내부 팀 테스트 |
| **P1** | 검색 에러 핸들링 | - | Qdrant/Gemini 실패 시 사용자 친화적 에러 |
| **P1** | Query Expansion/Rewriting | `docs/architecture/05-rag-pipeline.md` | LLM으로 질문 확장/재작성 |
| **P2** | Flutter 모바일 앱 | `docs/prd/16-app-feature-spec.md` | 레드팀 후 Phase 4 |
| **P2** | 사용자 인증 (Clerk) | - | Flutter와 함께 |
| **보류** | 종교 용어 사전 동적 주입 | `docs/specs/domain/06-terminology-dictionary-structure.md` | 데이터 미확보 |

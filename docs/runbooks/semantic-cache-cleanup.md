...
`apps/api/scripts/cleanup_semantic_cache.py` — standalone Python (raw httpx,
...
`apps/api/.env`가 자동 로드되므로 별도 export는 불필요하다. 운영 값은 복제하지 않으며 먼저 `--dry-run` 대상 환경을 확인한다.
...
- `docs/adr/55-semantic-cache-hardening.md` — 본 작업 ADR (SaaS 업계 비교 +
  결정 근거)
- `apps/api/app/modules/cache/service.py` — semantic cache 본 로직 (invalidation 메타데이터
  포함)
- `apps/api/scripts/cleanup_semantic_cache.py` — 본 가이드의 스크립트 본체

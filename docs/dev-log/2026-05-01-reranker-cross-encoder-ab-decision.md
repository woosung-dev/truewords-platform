...
### PR 11 추가 cleanup (2026-05-02): Protocol 단순화

사용자 추가 의견 ("기존 옵션 변경 X면 추가 코드도 단순화") 반영. swappable Protocol 자체가 의미 없는 단일 모델 환경 → `src/search/rerank/` 폴더 → `src/search/reranker.py` 단일 파일로 통합.

**Tier 2 추가 변경:**
- `src/search/rerank/{__init__.py, gemini.py}` → `src/search/reranker.py` 단일 파일 (function-based, class/Protocol 제거)
- `src/chat/pipeline/stages/rerank.py` — `get_reranker(...).rerank(...)` → `rerank(...)` 직접 호출
- `src/chatbot/runtime_config.py` — `reranker_model: Literal["gemini-flash"]` 필드 자체 제거
- `tests/search/rerank/{test_gemini.py, test_protocol.py}` 제거 (Protocol/factory 사라짐)
- `scripts/evaluate_threshold.py` — `from src.search.rerank import get_reranker` → `from src.search.reranker import rerank`
- 영향 받는 6 test files (test_chat_service / test_intent_routing / test_phase2_stages / test_cache_integration / test_evaluate_threshold / test_evaluate_threshold_collection) — patch path 및 mock setup 단순화 (`mock_reranker_inst.rerank` → `AsyncMock` 직접 patch)

**Tier 3 (측정 도구) 보존:** `compare_rerankers.py` / `evaluate_threshold.py` / `match_snippets_to_chunks.py` / `tests/golden/` 모두 유지 — 미래 reranker 도입 시 즉시 재측정 가능.

**효과:**
- 운영 동작 무변동 (Gemini reranker 동일 호출)
- 코드 단순화: 1 파일 (`reranker.py`) ~120줄 vs 이전 2 파일 + Protocol/factory ~200줄
- backend pytest 796 → 783 (Protocol/factory 단위 테스트 제거)

...
> 결정일: 2026-04-30
> 상태: 결정 완료, 구현 완료, prod 검증 완료 (PR #84 머지 `d8a14c2`)
> 관련 ADR: `45-qdrant-self-hosting.md`, `46-qdrant-cache-cold-start-debug.md`
> 관련 PR: #78 (진단), #83 (cache 전환), **#84 (chat search 전환)**
...
### 핫패스 (chat 응답)
✅ 본 PR 로 영구 해결. SDK 의 HTTP/2 hang 이 chat 경로에서 완전 제거됨.

### 웜패스 (admin / pipeline / startup) — 잔존 SDK 사용처
chat 응답에는 영향 없으나 관리 작업에서 hang 가능성:
- `backend/src/datasource/qdrant_service.py` — admin volume 태그/카테고리 통계 (8곳)
- `backend/src/pipeline/ingestor.py`, `batch_service.py` — 인제스트 시 upsert
- `backend/src/qdrant_client.py` — startup `create_collection` 동기 호출

후속 PR-D (admin), PR-E (pipeline + startup) 에서 점진 정리. 우선순위는 admin UI 의
hang 빈도에 따라 결정.
...
- PR #84: https://github.com/woosung-dev/truewords-platform/pull/84
- 머지 커밋: `d8a14c2`
- Cloud Run revision: `truewords-backend-00127-pgh`
- 변경량: +1,012 LoC, 9 파일

<!-- archify 로 생성한 아키텍처 다이어그램 4종의 원본·산출물·재생성 절차. -->
# 아키텍처 다이어그램

루트 `README.md` 가 참조하는 다이어그램 4종이다. 원본은 `.json` 이고, `.html` 은 자체 포함 인터랙티브 뷰어(pan/zoom · 검색 · guided view · 다크/라이트 · PNG/SVG export), `.png` 는 README 용 정적 캡처(2048×1320 light)다.
...
architecture 3종은 `meta.repository` + 컴포넌트별 `sources` 로 코드 경로를 박아 두었다. 검증 시 `--repo-root` 를 넘기면 경로 존재를 확인한다.

## 재생성

[archify](https://github.com/tt-a1i/archify) Skill 이 설치돼 있어야 한다 (`~/.claude/skills/archify`). 레포 루트에서:
...
node $A visual-check $D/system-architecture.html
```
...
| chat-request | pass — 9 checks, errors 0 / warnings 0 | 아래 갱신 |
...
- 그림은 코드를 따른다. 컨테이너 · 테이블 · 라우트가 바뀌면 `.json` 을 고치고 위 명령으로 다시 만든 뒤 `.png` 를 교체한다. PNG 만 고치지 않는다.

...
- `[확인 필요]` **"함께 머물렀어요" 반응 수 표시** — 프로토타입은 남의 한 줄에는 숨기고 내 한 줄에만 "N명이 함께 머물렀어요" 를 작게 보인다. 대안: 모두에게 작게 / 누구에게도 안 보임 (`DEC-PWA-023`, 2026-09-23)
- `[확인 필요]` **익명 숫자 숨김 기준 인원** — 예: 10명 미만이면 숫자 대신 "오늘도 식구들과 함께 읽었어요" (`?together=small`). 베타 초기 가입자가 적어 첫 주에는 이 문장이 기본이 될 가능성이 크다
...
- `[확인 필요]` **훈독 AI 질문 전용 봇·시스템 프롬프트**(PLAN-HD-002 W2) — **구현 완료**: 기존 봇 슬러그 `all` 재사용(무기억·근거 게이트·`/chat/stream` 스키마 무변경). 전용 봇·프롬프트가 정해지면 `apps/web/src/features/hoondok/ask/ask-stream.ts` 의 `HOONDOK_ASK_CHATBOT_ID` 상수 한 줄만 교체하면 된다. [PLAN-HD-002 §9](plans/active/2026-09-19-hoondok-screens.md)
- `[확인 필요]` **훈독 계정 삭제의 의미**(PLAN-HD-002 W0-B) — **구현 완료**(API-HD-011): `users.deleted_at` 기록 + 이메일 `deleted:{id}` 익명화(재가입 허용) + 본인 `mission_logs`·`jeongseong_periods` 하드 삭제. `users` 행 물리 삭제·보존 기간은 정책 확인이 남아 있고, 다른 정책이면 구현을 바꿔야 한다. [PLAN-HD-002 §9](plans/active/2026-09-19-hoondok-screens.md)
...
- `[종결]` **질문 상세 화자 칸** — PLAN-HD-005에서 “화자 확인되지 않음”을 명시하기로 확정.
- `[종결]` **훈독 오버스크롤 배경** — PLAN-HD-005에서 안 2(`overscroll-behavior-y: none`) 채택. 토큰 경계를 유지하며 Android 당겨서 새로고침은 비활성화된다.
- `[종결]` **설치 카드 조건** — 직접 완료(`source === "user"`) 이후 표시하는 현행 유지. 가입 직후 소급 완료는 제외.
...
- `[종결]` **홈 '이번 주' 메타** — PLAN-HD-005에서 summary 도착 이후에만 실제 연속일을 표시하기로 확정.
- `[종결]` **정성 기간·말씀 읽기 판정** — 7·21·40일 달력 종료일 및 read 완료 기준 유지. study는 원문 하단 읽음 버튼으로 완료하며 연속일에 포함하지 않는다.
- `[후속]` **결석 보호 아이템·알림** — 연속 참여 보상과 결석 시 보호 아이템 사용은 후속 고도화. 이번에 선행 테이블·동작을 구현하지 않는다. [참고 기능](https://blog.duolingo.com/how-duolingo-streak-builds-habit/)
...
- [ ] **구현 계획(PLAN-HD-0NN) 작성** — 1단계 익명 숫자는 `MissionLog` 만으로 새 테이블 없이 가능. 2단계 모임·멤버·모임별 이름·모임 정성·한 줄·반응 모델과 모임 초대 코드(전역 베타 코드와 별개)가 새로 필요하다. 아래 `[확인 필요]` 4건을 먼저 받는다

...
- `[확인 필요]` **데스크톱 내비: 레일 vs 헤더 1 문서형 / 2 필 그룹 / 3 감귤 밴드 / 4 검색 중심 / 5 플로팅 필 / 6 분할 내비** — 사용자가 A 기준 테마를 택한 뒤 내비만 비교(`board-nav.html`, `?nav=rail|top&header=1|2|3`). 헤더 1~3 은 `taste-skill` trust-first 프리셋, 4~6 은 12개 사이트 벤치마크 + `gpt-tasteskill` 내비 아키텍처로 추가. `DES-PWA-003` §4.1 은 레일, `DES-PWA-004` 도구 3종은 상단. [DES-PWA-005 §5.5](prd/2026-09-16-hoondok-designmd-directions.md)
- `[확인 필요]` **`DES-PWA-005` DESIGN.md 7안 중 무엇으로 갈 것인가** — C Notion·E Starbucks 동률 1위(84.3), F Claude 3위, A 기준 4위(73.3). E·C·F 는 `DEC-PWA-017` 재결정 + `DES-PWA-003` 토큰 교체가 필요하고, A 유지 시 E 의 반전 원문 블록·C 의 AI 이중 라벨·D 의 형태 전용 요일 칩 흡수를 권한다. 7안 공통 미해결 2건(인라인 링크 19.5px, 체크·칩 44px 미만)은 어느 안이든 같이 고친다. [DES-PWA-005 §5](prd/2026-09-16-hoondok-designmd-directions.md)

...
- `[확인 필요]` `docs/specs/web/hoondok-design-system.md` §4 컴포넌트 표(사진 히어로 행)가 아직 폐기된 단일 정지점 veil `0 → 0.78 100%` 를 적고 있다. §3.2 와 §4.6 이 정지점 3개로 바로잡은 값과 모순 — 사진 히어로 복원(2026-09-22) 때 발견했으나 범위 밖이라 손대지 않았다. 표만 고치면 되는지 확인 필요

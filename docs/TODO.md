...
### 가정연합 PWA 훈독(가안) PRD v2 + 디자인 2안 (2026-09-09)

- [x] PR #261 close 원인 정리 — PRD·디자인 문서에 가정연합 고유 개념 0건, 사용자 메모 미반영, 시안 완성도. 재작업 기준을 PRD v2 §0.2 에 기록
- [x] PRD v2 — 페르소나 3(2세 청년·축복가정 부모·교회장), 용어 사전(하늘부모님·식구·훈독·보고기도·정성·가정맹세·교회장), 초원AI 화면 23장 대응표, 5탭 `오늘·질문·말씀·가정·나`, `SCR-PWA-001~013`, `REQ-PWA-016~022`(오늘의 훈독 3카드·정성 기간·가정 단위·추천/북마크 질문·말씀 서고·5분 말씀·나의 훈독), KPI 북극성 = 주간 훈독 완료율
- [x] S0 비범위 반전 3건을 Decision Log 로 명시 — 5탭(`DEC-PWA-014`), 정성 일수(`-015`, 벌점·재화 없음), 가정 단위(`-016`)
- [x] 디자인 2안 `DES-PWA-002` — A 아침 햇살(따뜻한 흰 + 감 `#C2452B`, 카드 20/pill/12) · B 한지와 먹(종이 + 쪽빛 `#34518A`, 8px 단일, 명조 제목). 대비 실측 A 4.88/4.65/5.02, B 7.03/5.12/7.81. em-dash·이모지·보라·Inter·초원 팔레트 0. 추천 B + A 오늘 카드 ★5
- [x] 프로토타입 — iPhone 393×852 6화면×2, 단일 HTML(Pretendard·Noto Serif KR·Phosphor CDN), `capture-screens.mjs` 로 2배 PNG 12장 + 보드 2장. 콘텐츠는 `featured_malssum.json` 5장 + 원리강론 목차, 나머지 예시 표시
...
- `[확인 필요]` `DEC-PWA-017` 앱 이름 "훈독"(가안)의 상표·공식 "훈독회" 브랜드 사용 가능 여부. 후보 ★: 훈독 5 / 아침훈독 4 / 효정말씀 3 / 말씀함께 2 (PRD v2 §6.6)
- `[확인 필요]` `DEC-PWA-016` 가정 단위 함께 훈독에서 미성년 자녀 계정 정책(보호자 초대·동의·데이터 최소화). 확정 전 성인만 초대
- `[확인 필요]` 사용자 메모의 "가행국" 정확한 조직명과 가정예배 자료(순서·나눔 질문) 제공·검수 주체
- `[확인 필요]` 천력 절기(참부모님 탄신일·기원절 등) 달력 표시 여부와 날짜 출처. 확정 전 화면에 넣지 않음
- `[확인 필요]` 5분 말씀 교회장 섭외·신청 승인 주체와 영상 호스팅 채널(원본 미저장, 외부 링크만)
- `[확인 필요]` `DEC-PWA-015` 정성 일수 완료 조건: 3카드 중 1개 이상(제안) vs 3개 모두

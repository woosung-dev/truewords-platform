...
- `[확인 필요]` **데스크톱 내비: 레일 vs 헤더 1 문서형 / 2 필 그룹 / 3 감귤 밴드** — 사용자가 A 기준 테마를 택한 뒤 내비만 비교(`board-nav.html`, `?nav=rail|top&header=1|2|3`). 헤더 3안은 `taste-skill` trust-first 프리셋으로 재정의. `DES-PWA-003` §4.1 은 레일, `DES-PWA-004` 도구 3종은 상단. [DES-PWA-005 §5.5](prd/2026-09-16-hoondok-designmd-directions.md)

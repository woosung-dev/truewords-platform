...
- [ ] **사용자 origin `truewords.woosung.dev` canonical 전환 (A안, 2026-09-06 승인·진행 중)** — zone 공유로 `app` 같은 일반명은 프로젝트 접두어로 바꾸는 것이 맞다(2026-09-06 논의, ★4). 순서: Cloudflare 라우트 `truewords.woosung.dev → web:3000` 추가 → `WEB_URL=https://truewords.woosung.dev` 로 web·admin 재빌드 배포 + VM `.env` `WEB_FRONTEND_URL` + Dockerfile 기본값·문서 → Cloudflare Redirect Rule `app.woosung.dev/* → truewords.woosung.dev/$1` 301. 테스터 가이드 링크는 301 로 유지. `api`·`vdb` 는 내부용이라 후순위(★2).

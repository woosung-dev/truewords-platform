...
- [x] Playwright 캡처 16화면 × 2안 + 데스크톱·모바일 뷰포트·몽타주 = 38장 (`docs/prd/prototypes/screenshots/2026-09-10/`), 콘솔 오류 0. 비교·추천은 `docs/prd/2026-09-10-hundok-design-directions.md`(★ A 골격 + B 등불 홈·세리프 인용 흡수)
...
- [x] 토큰·치수는 시안 `2026-09-10-hundok-a.html` CSS에서 추출. 스테이지 배경 `--bg #f4f2ee`는 앱 토큰이 아니므로 제외, 미사용 `.serif-quote` 선언도 제외
...
### 훈독 데스크톱 시안 4안 + 블라인드 심사 (2026-09-16)
- [x] `DES-PWA-003` §4 데스크톱 규칙을 실제로 그려 검증 — 안 A 제작 중 **규칙 오류 5건 발견·수정** (2-pane 최소 폭 1224px · 화면 단위 단일 읽기 폭 · 앱바 폭 = 콘텐츠 폭 · 레일 "말씀" 아이콘 칩 · veil 정지점 3개). §4.6 신설
- [x] 비교용 대안 3안 제작 — B 감청 편집(`gpt-tasteskill`) · C 청화(`taste-skill`) · D 솔잎 조판(`ui-ux-pro-max`). 정보구조·콘텐츠 동일, 팔레트·서체·모서리·레이아웃만 재해석
- [x] 4안 × 8화면 = 32캡처 + 안 A 주석 제거판 8캡처. 가로 넘침 0px, 콘솔 오류 0
- [x] 독립 블라인드 심사 3명(빈 컨텍스트 opus, 도구명 비공개, 5축 × 20점) — **D 89.3 · C 80.0 · A 74.3 · B 73.3**. D 3표 만장일치 1위, C 3표 만장일치 2위. 원문 `docs/prd/prototypes/reviews-2026-09-16/`
- [x] 비교 문서 `DES-PWA-004` 작성 (`docs/prd/2026-09-16-hoondok-desktop-directions.md`) — 대비 계산 비교, 3개 도구의 수렴 선택 4가지, 심사 집계, 결정 가이드
- [x] 공정성 공개 — 안 A 에만 검증용 주석 박스 7개가 있어 심사원 2명이 감점. `a-clean-*.jpg` 별도 제공. 단 A 의 최대 약점(레이아웃 13.0)은 주석과 무관

### 훈독 DESIGN.md 7안 비교 + 독립 심사 (2026-09-16)
- [x] awesome-design-md(MIT) DESIGN.md 6종 사본 보관(`docs/research/design-md/`) + 훈독 도메인 번역 테마 CSS 6장. 마크업 한 벌 `hoondok-ds/app.html` 위에 `[data-theme]` 오버라이드만. 브랜드 기호 0
- [x] 심사 전 스킬 패스 2회 — `ui-ux-pro-max` 21건(E 탭 포커스 링 2.16:1 → 흰 링) · `taste-skill` 14건(F 균일 카드 → 헤어라인 행). 기각 사유는 CSS 하단 주석
- [x] 7안 × (폰 3 + PC 2) = 35캡처, 가로 넘침 0, JS 오류 0. 비교 보드 `board-designmd.html`(폭·화면·실제 크기 토글)
- [x] 독립 심사 3명(빈 컨텍스트 opus, 5축 × 20점) — **C Notion · E Starbucks 84.3 동률** · F Claude 78.7 · A 73.3 · D 68.7 · B 66.3 · G 63.3. 원문 `docs/prd/prototypes/reviews-2026-09-16-designmd/`
- [x] 심사 중 **A 기준안 결함 발견·수정** — `theme-card.css` 가 출처 라인을 모든 화면에서 숨겨 read·words 에 `O1 공식 원문` 배지가 없었다(이전 커밋 `4ecf6d0`). today 한정으로 수정 후 A 만 재채점(65/71/68 → 70/77/73, 순위 구조 불변)
- [x] 비교 문서 `DES-PWA-005` (`docs/prd/2026-09-16-hoondok-designmd-directions.md`) — 토큰·대비 표, 3명 공통 지적, 결정 가이드, 흡수 후보 4건

### 훈독 16화면 프로토타입 완성 + 결정 3건 (2026-09-16)
- [x] 사용자 결정: **A 기준 유지**, 데스크톱 **헤더 4(검색 중심)**, AI 질문 탭 **"물음 한 장"**(묻기 홈 + 기록 분리, FAB 제거). `DES-PWA-005` §5.5·§5.6, PRD `SCR-PWA-005`, `DES-PWA-003` §2.9·§8 반영
- [x] 단일 소스 `hoondok-ds/app.html`+`hoondok.css` 에 16화면 완성(폰 390 · PC 1280 동시). 원문 뷰 2-pane(1224+), 리더 바 상단 툴바, 정성 시트→모달, 검색 진입점 3종
- [x] 폴리시 패스: ui-ux-pro-max 2차 11건 + taste-skill 2차. 베이스 결함 3건 수정(레일 우측, display 가 hidden 덮음 ×2, 출처 라인 숨김)
- [x] 보드 `board-pc.html`(16화면), `board-ask.html`(채택안), 캡처 32장 `screenshots/2026-09-16-designmd/full/`
...
- `[종결]` `DEC-PWA-017` 디자인 방향 — **2026-09-14 A 아침 햇살 단독 채택**. B 저녁 등불 미채택, A+B 흡수 3항목(시간대별 홈·세리프 인용·다크 팔레트) 모두 미도입. 베타는 라이트 고정이며 다크 모드는 출시 후 재검토. [DES-PWA-002 §10](prd/2026-09-10-hundok-design-directions.md#10-결정-기록)
...
- `[확인 필요]` **`DES-PWA-004` 데스크톱 시안 4안 중 무엇으로 갈 것인가** — 심사 1위는 D 솔잎 조판(89.3)이나 `DEC-PWA-017`(A 아침 햇살 단독) 재결정이 필요하고, D 는 액센트 2색이 `--ok` 완료 시맨틱과 충돌해 해소가 필수다. A 를 유지하면 재결정은 없지만 데스크톱 레이아웃(13.0/20)을 손봐야 한다. [DES-PWA-004 §6](prd/2026-09-16-hoondok-desktop-directions.md)
- `[종결]` 데스크톱 내비 — **2026-09-16 헤더 4(검색 중심) 확정** — 사용자가 A 기준 테마를 택한 뒤 내비만 비교(`board-nav.html`, `?nav=rail|top&header=1|2|3`). 헤더 1~3 은 `taste-skill` trust-first 프리셋, 4~6 은 12개 사이트 벤치마크 + `gpt-tasteskill` 내비 아키텍처로 추가. `DES-PWA-003` §4.1 은 레일, `DES-PWA-004` 도구 3종은 상단. [DES-PWA-005 §5.5](prd/2026-09-16-hoondok-designmd-directions.md)
- `[종결]` `DES-PWA-005` 7안 — **2026-09-16 A 기준 확정**(심사 1위 C·E 는 참고). 흡수 후보(E 반전 원문 블록·C AI 이중 라벨·D 형태 요일 칩)는 구현 단계 검토. 원래 항목: — C Notion·E Starbucks 동률 1위(84.3), F Claude 3위, A 기준 4위(73.3). E·C·F 는 `DEC-PWA-017` 재결정 + `DES-PWA-003` 토큰 교체가 필요하고, A 유지 시 E 의 반전 원문 블록·C 의 AI 이중 라벨·D 의 형태 전용 요일 칩 흡수를 권한다. 7안 공통 미해결 2건(인라인 링크 19.5px, 체크·칩 44px 미만)은 어느 안이든 같이 고친다. [DES-PWA-005 §5](prd/2026-09-16-hoondok-designmd-directions.md)
- `[확인 필요]` **세리프 말씀 인용 재검토** — `DEC-PWA-017` 이 2026-09-14 기각한 항목인데 3개 디자인 도구가 독립적으로 모두 재제안했고, `DES-PWA-005` 상위 3안(C·E·F)도 전부 세리프 원문이다. 기각 근거였던 "장년층 가독성 확인"은 아직 수행하지 않았다
- `[확인 필요]` 좌측 레일 vs 상단 가로 내비 — `DES-PWA-003` §4.1 은 레일을 택했으나 3개 도구 모두 상단 가로 내비를 택했다. 5개 항목에 240px 세로 기둥이 과한지 `[가정]`
...
- [x] **레포 public 전환 (D1)** (2026-09-05 완료) — 사용자가 Settings 에서 전환. 직후 적용: ruleset `main-protection`(id 22336328 · PR 필수 · `CI Required` 필수 체크 · force-push/삭제 차단 → `protected: true`), Dependabot alerts, secret scanning + push protection, `delete_branch_on_merge: true`. 사전 점검 결과: 히스토리 grep 스캔에서 2026-03-28 초기 커밋의 `backend/.env`(Gemini 키) 1건 — 같은 날 제거됐고 **현재 무효**(models.list HTTP 400)라 회수 불필요.
  - [x] 개인 이메일 메모 — 2026-09-06 **일반화**: infra README·`.env.example` 은 계정 주소를 지우고 "운영자 개인 Google 계정(`jetaime-dev` 와 다름)" 으로, `apps/web/src/test/login.test.tsx` fixture 는 더미로, `docs/runbooks/redteam-test-guide.md` 계정·비밀번호 표는 플레이스홀더로 바꿨다. ADR·`docs/archive/` 이력 문서는 유지(git 히스토리에 같은 주소가 390회 남아 있어 실익 없음).
  - [x] 2026-04-29 private 복귀 이유 — 보안 후퇴가 아니다. Actions 무료 분 소진 우회로 4일 public 후 **계획된 복귀**(키 rotation·시크릿 재검사 선행). 근거 `docs/archive/engineering/42-deploy-pipeline-recovery-and-verification.md` §2.
  - [ ] `[확인 필요]` 레드팀 관리자 계정 비밀번호 실제 교체 여부 — 가이드는 "테스트 종료 시 교체" 라고 적혀 있었고 값은 public git 히스토리에 남아 있다. 미교체면 즉시 교체.
  - [ ] `[확인 필요]` **LICENSE 미결** — 파일 없음 → all-rights-reserved 묵시. 코드 라이선스와 별개로 **코퍼스 발췌 공개 범위**가 선행 판단이다: `apps/api/app/modules/malssum/featured_malssum.json`(원문 발췌 20건), `reports/baseline_*.jsonl`(답변 400건). 자체 리서치(`docs/research/2026-08-30-pwa-app-direction.md`)는 "615권 공개 가능 간주 금지". 옵션: 명시 all-rights-reserved 파일 ★5 / MIT + 콘텐츠 NOTICE ★2 / 파일 없이 유지 ★1. 히스토리 제거는 force-push 필요(룰셋 차단)라 비범위. 권리 확정 후 결정.
...
- [x] **DEMO_ADMIN_EMAIL 운영 반영 — backend 만** (2026-09-05 14:46 UTC) — VM `.env` 추가 → `make deploy-backend`(첫 `deploy-guard` 통과, `deploy.log` `deploy backend 446a4bf guarded`, /health 200 · SSE chunk/sources/done 확인). **admin 은 배포하지 않았다** — 운영 admin `30ca81f` 는 분리 전 통합 이미지라 새 admin 이미지로 바꾸면 채팅 화면이 사라진다(web 미배포). web/admin 분리 컷오버(DEC-MONO-002) 때 `make deploy-admin DEMO_ADMIN_EMAIL=<같은 값>` 을 함께 실행한다. → **2026-09-06 컷오버에서 실행 완료**(admin `41a9ef2`, `deploy.log` `deploy admin 41a9ef2 guarded`).
- [x] **ops-check `backup-remote` 오탐 수정** (2026-09-05) — 배포 전 점검에서 "원격 사본 209h 전" FAIL. 원인: 버킷 `truewords-backups` 를 quantbridge 와 공유하고 `oci os object list` 가 이름순 첫 100개(전부 `quantbridge/…`)만 돌려줘 우리 객체가 빠졌다. 실제 최신 사본은 12:00 UTC(153개)로 정상. `--prefix truewords- --all` 로 수정. `containers` 검사는 compose 정의에서 서비스 목록을 읽도록 바꿔(분리 전 5개·후 6개 공용) 현재 버전을 VM 에 배포할 수 있게 했다 — 배포 후 `make ops-check` 7건 OK 확인이 완료 조건.
- [x] **web/admin 분리 컷오버 — DEC-MONO-002 실행** (2026-09-06 완료, 채팅 무중단) — 순서·시각(UTC): VM 준비(compose·`.env` 백업, `preserve-images.txt` 에 `truewords-admin:30ca81f`·`truewords-backend:446a4bf`, `.env` `WEB_TAG=`·`WEB_FRONTEND_URL`·`ADMIN_FRONTEND_URL=https://truewords-admin.woosung.dev`, 6서비스 compose 전달) → `make deploy-backend` **e833ce9** 23:41(fastembed 0.8·cryptography 50 반영, SSE 확인) → `make deploy-web` **dfb6916** 23:46(컨테이너 내부 `/login`·CSS·SSE 검증, 동시 SSE 5건 부하에서 web 피크 72MiB/512MiB) → Cloudflare `truewords-admin.woosung.dev → admin:3000` 등록 → `app.woosung.dev` Service `admin:3000 → web:3000` 00:27(무중단, 10분 관찰 10/10 OK) → `make deploy-admin` **41a9ef2** 00:38(`--no-deps`, backend·web 재생성 없음) → `ops-check` **7건 OK, containers 6개**. 발견: 최초 `deploy-web` 이 backend 를 재생성(env_file 해시) → #239 `--no-deps` 수정 후 admin 배포. 롤백 태그 `truewords-admin:30ca81f` 는 `preserve-images.txt` 로 2주 이상 보존. 관리자는 `truewords-admin.woosung.dev` 에서 재로그인(host 별 쿠키). 기록: [전환 runbook §실행 기록](runbooks/monorepo-migration-and-rollback.md#실행-기록-2026-09-06).
- [x] **사용자 origin `truewords.woosung.dev` canonical 전환** (2026-09-06 완료) — zone 공유로 프로젝트 접두어 통일(★4). Cloudflare `truewords → web:3000` 추가 → #241(Dockerfile 기본값·문서) → VM `.env` `WEB_FRONTEND_URL` + backend 재시작 24초 → `make deploy-web`/`deploy-admin` **8980e0c** 재빌드(`--no-deps`) → admin `/history` 307 → `truewords` 확인 → **`app.woosung.dev` route·DNS 삭제**(실사용자 없어 Redirect Rule 301 생략, 사용자 결정). 남은 route: api·vdb·truewords-admin·truewords. 발견: `docker save \| ssh` 전송이 ssh 끊김으로 55분 정지 → 파이프 종료 후 재실행. **후속 수정**: `make deploy-*` 전송을 로컬 tgz + `rsync --partial` + VM `docker load` 로 분리(재시도 가능). 기록: [runbook §실행 기록](runbooks/monorepo-migration-and-rollback.md#실행-기록-2026-09-06).
...
- [x] 세션 2' — 디자인 2안 클릭형 목업 작성 (`docs/prd/prototypes/2026-09-10-hundok-{a,b}.html`)
...
- [x] **admin Oracle 이전 + 컷오버** (2026-07-30) — Next.js `output: "standalone"` 컨테이너로 VM 이전. 접속 주소 `https://app.woosung.dev`. `NEXT_PUBLIC_API_URL` 은 rewrites 가 빌드 타임에 구워지므로 build ARG (`http://backend:8080` — Cloudflare 왕복 1회 절감). Vercel 은 host 조건부 307 리다이렉트 전용으로 존치했다가 2026-09-05 프로젝트 삭제로 종료.
...
- [x] **예약 작업 실패 알림 — 전달 채널** (2026-09-05 결정: **ntfy.sh**) — 탐지만 있고 전달이 없어 2026-08-07~31 재발(25일)을 놓친 뒤 닫았다. `ops-check.sh` 마지막에 FAIL/WARN 일 때만 `https://ntfy.sh/$NTFY_TOPIC` 으로 푸시 1건. 계정·IAM 없음, VM `.env` 의 `NTFY_TOPIC` 한 줄. 상세: `infra/oracle-vm/README.md` §전달, ADR 후속 절.
  - [ ] **수신 방식 결정 `[확인 필요]`** — 사용자는 폰에 ntfy 앱을 설치하지 않기로 했다(2026-09-05). 코드의 ntfy 블록은 `NTFY_TOPIC` 미설정이면 아무것도 하지 않으므로 그대로 두고, 아래 중 하나를 고른 뒤 커밋 하나로 붙인다.
    | 후보 | ★ | 직접 할 일 | 비고 |
    |---|---|---|---|
    | **healthchecks.io** | ★5 | 이메일 가입 + 체크 1개 생성, ping URL 전달 | 이메일 알림 기본. **cron 자체가 안 돈 것**까지 잡음(dead-man). 무료 20개 체크·로그 100건. `ops-check.sh` 끝에 성공 `curl $HC_PING_URL`, 실패 `…/fail` + 본문 |
    | ntfy.sh 이메일 전달 | ★4 | ntfy.sh 웹 계정 + 이메일 인증 | 같은 curl 에 `Email:` 헤더 한 줄. 익명 이메일 전송은 남용으로 막혀 있고 무료 하루 5통. dead-man 못 잡음 |
    | Discord/Telegram 웹훅 | ★3 | 이미 쓰는 메신저의 웹훅 URL | 새 앱은 아니지만 계정 필요 |
    | OCI Notifications 이메일 | ★3 | OCI 콘솔에서 토픽·이메일 구독·IAM 정책(dynamic group → ons) | 계정 추가 없음(Instance Principal), 설정이 가장 김 |
  - [ ] VM 반영 — 채널 확정 후: `scp infra/oracle-vm/ops-check.sh truewords-oracle:~/truewords/`, `.env` 에 채널 변수 추가.
  - [ ] 반증 리허설 — `make ops-check`(알림 없음) → `ssh truewords-oracle 'BACKUP_MAX_AGE_H=0 bash ~/truewords/ops-check.sh'`(알림 도착, 본문 `backup FAIL`).

...
- [x] 차별화 제품 방향, MVP/비범위, S0~S13의 14개 세션 로드맵과 S1~S4 후속 프롬프트 작성 (`docs/research/2026-08-30-pwa-app-direction.md`)
...
- [x] 세션 1 PRD 작성 — `REQ-PWA-001~015`, O1~O5/R, 권리 원장, KPI, G0~G5, S2 검증 가설 정의 (`docs/01_requirements/17-ffwpu-pwa-prd.md`, 사용자 검토 대기)
- [x] 사용자 후속 결정 반영 — 기존 S5+S6→새 S5, 기존 S9+S10→새 S8로 통합하고 권리 원장→RAG→통합 검증→베타 배포→결과 판정 분리
...
### 가정연합 신규 PWA 출시 게이트 (S6 콘텐츠 적재·S12 베타 배포 차단)

- [ ] `DEC-PWA-001` 독립 베타의 법적 운영 주체와 FFWPU 공식 승인 절차 확정 — 선택지·추천안·결정 시점은 PRD §5.5 [확인 필요]
- [ ] `DEC-PWA-002` 초기 소규모 정본의 정확한 목록과 기능별 권리 증빙 확정 — 선택지·추천안·결정 시점은 PRD §5.5 [확인 필요]
- [ ] `DEC-PWA-003` 콘텐츠 공식성·검수·철회 최종 책임자 확정 — 선택지·추천안·결정 시점은 PRD §5.5 [확인 필요]
...
- [x] 세션 1 — PRD 작성 및 자체 검증 완료 (`docs/01_requirements/17-ffwpu-pwa-prd.md`)
- [ ] 세션 1 승인 — 제품 정의·MVP/비범위·`[제안]` KPI/보존 정책·G0~G5 사용자 검토
- [ ] 세션 2 — **PRD 명시적 승인 후에만** A/B/C 비교형 프로토타입 작성·방향 선택
- [ ] 세션 3 — 채택안 디자인 시스템·접근성 상태 작성·승인
- [ ] 세션 4 — 구현 설계·작업 분해·제한 베타 계획과 S5~S13 실행 runbook 작성
- [ ] 세션 5~13 — 승인 runbook 순서로 구현·검증·독립 베타·결과 판정

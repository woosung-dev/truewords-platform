...
청사진 (`docs/architecture/target-architecture-blueprint-2026-05-01.html`) 이 적신호로 표시한 "score_threshold 0.60" 버그도, 본 ADR 작성 직전 `plan.md` 가 가정한 "절대값 0.75 → 상대값 top × 0.7 전환" 도 **둘 다 잘못된 전제**. 실제 cutoff 가 무엇인지 코드 trace 로 확인한 결과만이 정확하다.
...
- `docs/architecture/target-architecture-blueprint-2026-05-01.html` — 외부 청사진 사본
- `docs/adr/24-rrf-score-threshold-fix.md` (2026-04-11) — 이전 RRF score threshold 작업 ADR (문맥 참고)
- `docs/adr/2026-05-01-target-architecture-gap.md` — 청사진과 실제 코드 차이 표

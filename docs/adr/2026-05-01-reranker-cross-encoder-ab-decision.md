...
상세: [`./2026-05-01-reranker-ab-results.md`](../research/2026-05-01-reranker-ab-results.md) (raw JSON: `.json`).
...
상세: [`./2026-05-01-reranker-ab-results-round2.md`](../research/2026-05-01-reranker-ab-results-round2.md)
...
## 12. 운영 코드 / 측정 도구 미머지 결정 (2026-05-02)

ADR 결정 (Gemini-flash-lite 유지) 확정 후 사용자 정책 ("기존 옵션 변경 X면 추가 코드도 단순화") 에 따라, 본 reranker A/B 작업의 main 머지는 **본 ADR 문서 + R1/R2 측정 결과 markdown/json 5 파일만** 으로 한정. 운영 코드 / 측정 도구 / 단위 테스트 / Dockerfile bake-in 모두 main 미반영.

### 미머지 항목 (운영 코드 변경 0, 의사결정 trace 만)

다음은 dev/reranker-cross-encoder 통합 브랜치에서 검증/측정 위해 사용된 코드지만, ADR 결정 ("default 변경 없음") 의 일관성 위해 main 미반영:

- `backend/src/search/reranker.py` 의 `_parse_scores` partial response 허용 — 측정 환경 (search_top_k=20) 전용 변경, 운영 chat 핫패스 (search_top_k=50) 의 graceful degradation 은 기존 코드로 충분
- `backend/src/chat/pipeline/stages/rerank.py` 의 `rerank_score_dist` 로깅 + ChatContext.results_before_rerank 필드 — 운영 모니터링 가치 있으나, ADR 백로그 #1 별도 PR 로 다루기 위해 일단 미반영
- `backend/src/chatbot/runtime_config.py` 의 `reranker_model` 필드 — single 모델 환경에선 불필요
- `backend/scripts/{compare_rerankers, evaluate_threshold, match_snippets_to_chunks, import_notebooklm_to_golden}.py` — 측정 도구
- `backend/tests/golden/queries.json` 60 entry + raw v1/v2 — 골든셋
- `backend/Dockerfile` BGE multi-stage bake-in
- `backend/pyproject.toml` sentence-transformers 의존성

### 측정 재현 가능성

본 측정의 인프라 / 코드 / 골든셋은 git history 의 dev/reranker-cross-encoder 브랜치 (PR #109~#123) 에서 영구 보존. 미래 reranker 재측정 시:

1. dev/reranker-cross-encoder 분기점에서 측정 인프라 cherry-pick
2. 또는 ADR 의 측정 결과 raw JSON / markdown 을 baseline 으로 재구축

### 머지 항목 (docs only)

- `docs/adr/2026-05-01-reranker-cross-encoder-ab-decision.md` (본 ADR)
- `docs/research/2026-05-01-reranker-ab-results.json` (R1 raw)
- `docs/research/2026-05-01-reranker-ab-results.md` (R1 분석)
- `docs/research/2026-05-01-reranker-ab-results-round2.json` (R2 raw)
- `docs/research/2026-05-01-reranker-ab-results-round2.md` (R2 분석 + R1/R2 비교)

### 효과

- 운영 동작 무변동 (default = `gemini-flash`, 코드 변경 0)
- main 의 코드 복잡도 ↑ 0
- 의사결정 근거 (5 docs) 는 영구 보존
- 미래 재측정: dev/reranker-cross-encoder 브랜치 + git history 활용

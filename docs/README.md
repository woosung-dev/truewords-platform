...
현재 작업은 **훈독 운영 공백 복구**다. 훈독 MVP 는 Phase 1(#276)·Phase 2(#282)·Phase 3 편성 트랙(#287)·PWA 트랙(#298, SW 킬스위치 후속 #299)·화면 확장 [PLAN-HD-002](plans/active/2026-09-19-hoondok-screens.md)(#300, main `4e15f8c`)까지 main 에 머지됐고, 운영 web `aba5240` 은 **`HOONDOK_ENABLED=1` 로 이미 배포돼 있다**(2026-09-20 실측 — `/hoondok`·`/hoondok/read`·`/hoondok/onboarding` 200, `x-robots-tag: noindex, nofollow`, `smoke-web` 12건 OK). 운영 web 태그는 main 보다 2커밋 뒤라 PLAN-HD-002 의 13화면은 아직 노출되지 않는다(`/hoondok/garden` 404). 편성 재고는 2026-09-20 에 2일분이 투입돼 `ops-check` 가 초록이 됐고(완료 기준 7일분에는 미달), 초대 코드 게이트는 끈 채로 유지하기로 결정했다 — 실측값·결정·위험은 [훈독 PWA 롤아웃 runbook](runbooks/hoondok-pwa-rollout.md) 의 실행 기록이 소유한다. 편성 재고를 지속적으로 채우기 위해 [PLAN-HD-003](plans/active/2026-09-20-hoondok-curation-assist.md) 편성 후보 찾기(추출형)를 구현했다 — 코퍼스 원문을 검색해 폼을 채우며 생성 AI 가 본문을 만들지 않는다. 2026-09-20 backend·admin 을 `c066b02` 로 배포해 운영 편성 화면에서 쓸 수 있다(web 은 `aba5240` 유지). 남은 것은 편성 5일분 추가다. 프리뷰 플래그는 운영에 배선하지 않는다. 앱별 UI 소유권 분리(2안)는 완료된 전제이며, web 태그 승격·알림·Flutter 는 별도 승인이다.

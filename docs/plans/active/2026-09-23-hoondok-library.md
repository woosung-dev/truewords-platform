...
| 8 | TTS · 판본 나란히 보기 | **비범위.** 판본 데이터·음성 인프라 없음. TODO 후속. TTS 는 → [`PLAN-HD-008`](2026-09-23-hoondok-reader-polish.md) 에서 브라우저 TTS 로 대체 | 사용자 결정 |
...
| C admin | `feat/hoondok-library-admin` (A 계약 스택) | `apps/admin/src/app/(dashboard)/hoondok/rights/**` · `apps/admin/src/features/hoondok/**` · admin Vitest · `tests/e2e/hoondok-curation.spec.ts`(권리 절 추가) | 시리즈 요약 렌더 · 일괄 다이얼로그 → bulk 페이로드 · 감사 로그 확인 | ✅ 머지 `93fee3e` — admin Vitest 127(+8), `@base-ui/react/dialog` 기존 패턴, E2E `hoondok-rights.spec.ts` 등록(통합 뒤 실행) |
...
| 2026-09-23 | 독립 리뷰 1회(Opus read-only, codex 한도) — P0 0 · P1 4 · P2 9 · 테스트 공백 7. P1 4건 + P2-1(철회 행 재개방) + 공백 6건 수정 `282783b`: 시리즈 화면 비허용 권 비링크 · `?section=` 요청 장 우선 + 시작 단락 스크롤 · marks/positions 원장 1회 조회 + `limit≤200`(계약 additive) · 시드 `--grade` choices · admin bulk/series 401 잠금 테스트. 재게이트 `make ci` EXIT 0(pytest 1218 · web 309 · admin 127) · `make e2e` 94 · 라이브에서 `?section=6` 머리말 "참부모"·`aria-current`·단락 323 스크롤 확인. 잔여 P2 2~9 는 TODO 후속 | 게이트 5 통과 → 6 dev→main PR(push 승인 대기) |
| 2026-09-23 | 운영 개통 뒤 Playwright 점검(1280·390, QA 계정) — 결함 3건 수정 `fix/hoondok-reader-jump-markdown`: ① 검색·북마크 `?chunk_id=` 가 해당 단락으로 스크롤하지 않음 → 목차와 같은 스크롤 대상에 포함 ② AI 설명·AI 질문 답의 `###`·`**` 노출 → 공용 `AnswerMarkdown`(react-markdown, 링크는 글자만) ③ 점프 단락이 sticky 앱바에 가려짐 → `.verse` `scroll-margin-top` 76px / lg 148px. web Vitest 311 · 로컬 dev(운영 API)에서 세 건 PC·모바일 확인. 후속 `[확인 필요]`: AI 설명 참고 말씀이 비공개 말씀선집을 인용 | 수정 완료, PR 대기 |

# 훈독 앱 PRD v2 — 가정연합 식구를 위한 말씀·훈독·가정예배 PWA

- 문서 ID: `PRD-FFWPU-PWA-001` (v2)
- 최초 작성: 2026-08-31 (PR #218 초안) · v1 재작성: 2026-09-07 (PR #261, close) · **v2 전면 재작성: 2026-09-10**
- 상태: **2026-09-16 승인** (S2' 방향 확정 2026-09-14 `DEC-PWA-015·016·017·019` → S3 `DES-PWA-003` → S4 구현 계획 [PLAN-HD-001](../plans/active/2026-09-17-hoondok-mvp.md))
- 제품 단계: 독립 운영·비공식 제한 베타 준비. 9월 중 기획 완료 목표
- 앱 이름: **훈독** (2026-09-14 확정, `DEC-PWA-016`). FFWPU 공식 승인 절차는 `DEC-PWA-001`의 외부 게이트로 남는다
- 함께 볼 것: [프로토타입 `hoondok-ds`](prototypes/hoondok-ds/README.md) (20화면, 폰·PC 단일 소스), [디자인 시스템 `DES-PWA-003`](../specs/web/hoondok-design-system.md)
...
| 정보구조 | 3탭 | **5탭** (오늘 훈독 · AI 질문 · 말씀 · 가정예배 · 나의 정원) | `DEC-PWA-015` |
...
성장의 문제도 하나 더 다룬다. **P6 나의 신앙 생활이 어디쯤인지 보이지 않는다** → "나의 정원"에서 미션 달력·누적일·정성 진행·가족·친구 관계를 한 화면에 보여 준다 (F5). 점수로 사람을 줄 세우지 않는다.
...
| AI 질문 | **AI 질문** | ~~추천 / 북마크 / 내 질문 3탭, 질문 카드 목록, 우하단 "+ 질문하기"~~ **2026-09-16 정정:** 탭 홈은 묻기 하나("물음 한 장"), 추천·북마크·내 질문은 앱바 "기록" 화면으로 분리, FAB 없음 (`DES-PWA-003` §8). 내 질문의 날짜·비공개 표시·삭제는 기록 화면에서 | 답변 화면에 **근거 말씀 카드(화자·연도·저작물·판본·공식성)** 와 **연관 말씀 → 이어지는 질문** 루프 추가. AI 설명과 공식 원문을 라벨·형태로 분리. 저장/무기억 선택 |
...
| 나의 초원 | **나의 정원** | 프로필·대표 성경구절, "주님과 함께한 시간"(현재 연속일·최대 연속일·누적일), 월 달력 체크, 설정(알림 시간·인기 질문·공지) | **대표 말씀**, 미션 달력, 정성 진행, **가족 관계**(부모·자녀·배우자 초대)와 **친구**, 알림 설정(중립 문구·조용한 시간). 멤버십·선물권·초대 코드 보상은 비범위 |
...
| 묵상하기 카드 "+5 달란트", "128명" | ~~훈독하기 카드 "우리 교회 N명 함께"~~ **2026-09-23 정정:** 홈 "오늘 새벽 함께 읽은 식구 N명"(익명) + 초대 모임 "오늘 5명이 함께 읽었어요"(완료자만) (`DEC-PWA-023`) | 보상 대신 함께 읽는 사람 수로 동기. 교회 단위 숫자는 교회 간 비교가 되므로 쓰지 않는다 |
...
| 나의 초원: 대표 성경구절·연속일·달력 | 나의 정원: 대표 말씀·연속일·누적일·달력·정성·가족·친구 | 관계를 전면에 |
...
```
오늘 훈독 · AI 질문 · 말씀 · 가정예배 · 나의 정원
```

가운데 탭은 초원처럼 돌출형 "말씀"으로 두어 앱의 중심이 말씀임을 시각화한다. 5탭 명칭은 2026-09-14 확정됐다 (`DEC-PWA-015`). 5번째 탭은 **"나의 정원"**이며 검토 대안이던 "나의 뜰"·"나의 훈독"·"내 정성"은 미채택이다.

### 3.2 화면 목록

| ID | 화면 | 탭 | 담당 과업 | 접근 |
|---|---|---|---|---|
| `SCR-PWA-001` | 온보딩·베타 고지·시작 방법 | 진입 | 독립 베타 고지, 로그인, 혼자 시작 / 초대 코드. **2026-09-23**: 소속 교회 선택 제거 (`DEC-PWA-023`) | 공개 |
| `SCR-PWA-002` | 오늘 훈독 (홈) | 오늘 | `JTBD-001` 인사·요일·연속일·미션 3종·정성 카드·함께 읽는 사람들(익명 숫자 + 내 모임) | 공개 콘텐츠는 비로그인 |
...
| `SCR-PWA-005` | AI 질문 (묻기 홈) + 질문 기록 | AI 질문 | `JTBD-003`. **2026-09-16 변경**: 탭 홈은 묻기 한 가지(오늘 말씀 한 줄 + 라벨 있는 입력 + 시작 문장 3개가 입력창을 채움). 추천/북마크/내 질문은 앱바 "기록" 화면으로 분리. FAB 없음. 프로토타입 `prototypes/hoondok-ds/app.html?screen=ask` | 묻기·추천은 비로그인, 기록은 로그인 |
...
| `SCR-PWA-011` | 챌린지 상세 | 가정예배 | `JTBD-007` 참여·진행률·오늘 읽은 참여자. 응원은 2026-09-23 제거 | 로그인 |
| `SCR-PWA-012` | 5분 설교 · 전체 설교 | 가정예배 | `JTBD-008` 교회장 행, 이번 주, 인기 | 비로그인 |
| `SCR-PWA-013` | 설교 섭외·신청 폼 | 가정예배 | 교회장 선택·주제·희망일·형식(5분/전체) | 로그인 |
| `SCR-PWA-014` | 나의 정원 | 나의 정원 | 프로필·대표 말씀·연속일·누적일·달력·정성·가족·친구 | 로그인 |
| `SCR-PWA-015` | 알림·설치 설정 | 나의 정원 | 알림 4종·시간·조용한 시간·문구 수준·설치 안내 | 로그인 |
| `SCR-PWA-016` | 가족·친구 관계 | 나의 정원 | 가족 초대·관계 표시·친구 목록·공개 범위 | 로그인 |
| `SCR-PWA-017` | 훈독 모임 상세 | 오늘 | `JTBD-007` 오늘 범위·정성 N일차·오늘 읽은 사람(완료자만)·한 줄 나눔·초대 (`DEC-PWA-023`) | 로그인·모임 멤버 |
| `SCR-PWA-018` | 모임 참여 | 오늘 | 초대 코드/링크 → 미리보기 → 모임별 표시 이름 → 공개 안내 | 로그인 |
| `SCR-PWA-019` | 모임 만들기 | 오늘 | 소그룹 모임·이름(별칭)·모임 정성 → 초대 코드·카카오톡 공유. 오늘 범위는 공식 편성 고정(2026-09-23) | 로그인 |
| `SCR-PWA-020` | 한 줄 나눔 쓰기 | 오늘 | 100자 한 줄, 모임에만 보임 | 로그인·모임 멤버 |
| `SCR-PWA-021` | 모임 설정 | 오늘 | 리더: 모임 이름·식구 목록(전체 인원은 여기서만, 읽음 상태 없음)·초대 코드 새로 만들기·내보내기·모임 삭제. 모임원: 내 이름·나가기 (`PLAN-HD-010`) | 로그인·모임 멤버 |

프로토타입 [`hoondok-ds/app.html`](prototypes/hoondok-ds/app.html)이 21화면을 모두 담는다. 화면 ↔ `data-screen` 대응은 그 파일 상단 목차 주석에 있다.
...
**화면 흐름.** 홈 상단에 시간대별 인사 문장(예: "밤이 깊을수록 새벽은 가까워요") → 요일 스트립(오늘 강조) + 연속일 → 미션 카드 3개 **훈독하기 · 기도하기 · 말씀 읽기** (각각 완료 체크) → 정성 기간 카드(진행 중이면 D-day·진행률, 없으면 "정성 시작하기") → ~~"우리 교회 N명 함께 훈독 중"~~ **함께 읽는 사람들**(익명 숫자 → 내 모임, F8 · 2026-09-23 `DEC-PWA-023`). 훈독하기를 누르면 오늘 말씀 본문(예상 읽기 시간, 화자·연도·저작물·판본·공식성) → 용어 칩 → 한 줄 성찰(선택) → 완료.
...
> **2026-09-16 정정.** 아래 "목록" 단락의 3탭 + 우하단 "질문하기" 구조는 폐기했다. `SCR-PWA-005` 탭 홈은 **묻기 한 가지("물음 한 장")** — 오늘 말씀 한 줄 + 라벨 있는 입력 + 시작 문장 3개가 입력창을 채운다. 추천·북마크·내 질문은 앱바 "기록" 화면으로 분리하고 FAB 은 없다 (`DES-PWA-003` §2.9·§8, §3.2 화면 목록 반영). 상세 화면 흐름은 그대로다.

**화면 흐름.** 목록(폐기, 위 정정 참조): **추천 · 북마크 · 내 질문** 3탭 + 검색 + 우하단 "질문하기". 추천은 운영 큐레이션 + 인기 질문(익명 집계), 북마크는 내가 저장한 질문, 내 질문은 날짜·공개 여부·삭제. 상세: 질문 → AI 설명(점선 배경, "AI 설명 · 공식 해설 아님") → **근거 말씀 카드**(굵은 왼쪽 선, 화자·연도·저작물·판본·공식성, 원문 열기) → **연관 말씀** 2~3건 → **이어지는 질문** 칩 3개(탭하면 새 질문으로 이어짐) → 하단 저장/무기억 토글·북마크·공유.
...
**초원 대비.** ~~3탭 구조 동일.~~ 초원의 3탭 목록 대신 묻기 홈 + 기록 분리(2026-09-16 정정). 근거 카드·연관 말씀·이어지는 질문 루프가 추가. AI 해설 잠금 없음.
...
- `AC-020-01`: 챌린지 단위는 가족·~~교회~~·소그룹이며 개설자는 ~~교회장 또는~~ 모임 리더 또는 성인 가족 구성원이다. 공개 챌린지는 비범위. **2026-09-23 정정 (`DEC-PWA-023`):** 교회 단위는 만들지 않는다(소속을 받지 않는다). 첫 단위는 소그룹 훈독 모임(F8)이고 가족은 후속이다.
- `AC-020-02`: 진행률·참여 인원은 표시하되 개인 순위·랭킹은 두지 않는다 (`DEC-PWA-019` 2026-09-14 확정). **2026-09-23 추가:** 참여자 목록에는 오늘 읽은 사람만 보이고 미완료자의 이름·상태는 어디에도 없다.
- ~~`AC-020-03`: 응원은 사전 정의 문구만 허용해 자유 댓글·DM을 열지 않는다.~~ **2026-09-23 `AC-024-04` 로 대체:** 사람에게 보내는 응원은 없다. 반응은 한 줄 나눔에 대한 "함께 머물렀어요" 1종이며 알림을 보내지 않는다. 자유 댓글·DM 은 계속 없다.
...
### F5. 나의 정원 (`REQ-PWA-007` 승계 + `REQ-PWA-022`, `P1`)

**목적.** 나의 신앙 생활이 어디쯤인지 보이게 하고, 가족·친구와 연결한다 (P6).

**화면 흐름.** 프로필(닉네임·소속 교회·대표 말씀) → "말씀과 함께한 시간"(현재 연속일·최대 연속일·누적일) → 월 달력(미션 완료일 체크) → 진행 중 정성 → **가족**(배우자·자녀·부모 초대, 관계 표시, 오늘 읽은 가족만 표시 — 2026-09-23 `DEC-PWA-023`) → **친구** → 설정(알림·설치·데이터 삭제·로그아웃).

- `AC-022-01`: 가족 관계는 양쪽 동의로 성립하고, 상대의 활동 표시 범위를 각자 설정한다. 기본은 ~~"오늘 완료 여부만"~~ **"오늘 읽은 날만"**(2026-09-23 정정: 읽지 않은 날은 표시하지 않는다, `DEC-PWA-023`).
...
- `AC-023-02`: 하루를 놓쳐도 정성은 끊기지 않고 ~~"밀린 날"로 표시하며~~ 진행은 "N일차"로만 표시하며(2026-09-23 정정, `DEC-PWA-023`), 사용자가 이어서 읽을지 다시 시작할지 고른다.
...
### F8. 함께 읽는 사람들 · 훈독 모임 (`REQ-PWA-024`, `P1`, 2026-09-23 `DEC-PWA-023`)

**목적.** "혼자가 아니다" 를 주되 감시·비교가 되지 않게 한다 (P4, `JTBD-007`). 근거: [2026-09-23 벤치마크](../research/2026-09-23-hoondok-together-benchmark.md).

**화면 흐름.** 1단계: 홈 "오늘 새벽 함께 읽은 식구 N명"(익명) · 훈독 완료 뒤 "당신까지 N명". 2단계: 리더가 소그룹 모임을 만들고(`SCR-PWA-019`) 초대 코드/카카오톡 링크를 보낸다 → 초대받은 사람이 모임별 표시 이름을 정해 참여(`018`) → 모임 상세(`017`)에서 오늘 범위·정성 N일차·오늘 읽은 사람·한 줄 나눔(`020`)을 본다. 프로토타입 `app.html?screen=group` · `?together=default|small|none`.

- `AC-024-01`: 익명 숫자는 오늘 훈독을 완료한 사람 수만 센다. 기준 인원 미만이면 숫자 대신 "오늘도 식구들과 함께 읽었어요" 를 쓴다 `[확인 필요]` 기준 인원. 숫자를 푸시에 넣지 않는다.
- `AC-024-02`: 모임 안에서는 오늘 읽은 사람의 모임별 이름과 시각만 보인다(가나다순). 미완료자의 이름·상태·"아직", 개인별 출석표, 독촉 알림, 순위·점수·보상은 없다.
- `AC-024-03`: 한 줄 나눔은 100자, 모임 멤버에게만 보이며 작성자가 지울 수 있다. 댓글·피드·좋아요 수 경쟁은 없다.
- `AC-024-04`: 반응은 "함께 머물렀어요" 1종이며 알림을 보내지 않는다. 반응 수 표시 방식은 `[확인 필요]`(프로토타입: 내 한 줄에만 작게).
- `AC-024-05`: 오늘 범위는 공식 편성(협회 공지를 편성자가 등록)을 따르거나 리더가 원문 단락 범위를 지정한다(리더 지정은 2026-09-23 이번 범위 제외 — 모든 모임이 공식 편성을 따른다, `PLAN-HD-010`). 정성은 공식 정성 + 모임 정성이며 "N일차 / 전체" 로 모임 전체 진행만 보인다. 빠진 날은 세지 않는다.
- `AC-024-06`: 모임 이름은 별칭이며 교회명을 요구하지 않는다. 앱은 소속 교회를 받지 않고, 교회 간 비교·랭킹·모임 검색을 만들지 않는다. 참여 전에 공개 범위("오늘 읽었는지와 내가 남긴 한 줄만. 형광펜·노트·질문은 항상 나만")를 보여 준다.
- `AC-024-07`: 첫 모임 종류는 소그룹(훈독가정교회)이다. 가족 모임은 후속이다 `[확인 필요]` 시점. 모임 초대 코드는 전역 베타 초대 코드와 별개다.
...
| 가족·~~교회~~·소그룹 챌린지 진행률·참여 인원 | 포함 (교회 단위는 2026-09-23 제외, `DEC-PWA-023`) | 함께 읽는 감각 |
| 개인 랭킹·순위 | **제외 확정** (2026-09-14 `DEC-PWA-019`) | 신앙 생활을 경쟁으로 만들 위험. 초원 그룹 통독의 "구성원별 순위"는 복제하지 않음 |
| 달란트형 화폐·보상 배지 | **제외 확정** (2026-09-14 `DEC-PWA-019`) | 초원 리뷰의 유료화·보상 반발 관찰. 도입한다면 헌물·결제와 분리된 비화폐 배지만 |
| 씨앗/나무 성장 메타포 | 제외 | 초원 고유 표현. 훈독은 "정원"에 달력·정성으로 성장을 표현 |
...
관리자 계정은 일반 사용자 기록을 기본 열람할 수 없다. JWT·Push payload·Service Worker 캐시·분석 이벤트에 질문·메모·가족 정보를 복사하지 않는다. `session_id` 재사용 경로의 쓰기 소유권 검증 부재(`SEC-MONO-001`)는 2026-09-16 PR #272 로 해결했다(불일치 시 403).
...
| MVP (제한 베타) | 비범위 |
|---|---|
| 5탭 전체: 오늘 훈독·AI 질문·말씀·가정예배·나의 정원 | 네이티브 앱, 완전 오프라인 RAG |
| 미션·연속일·정성·가족/소그룹 챌린지, 익명 함께 읽은 수·초대 소그룹 모임·한 줄 나눔 (`DEC-PWA-023`) | 공개 커뮤니티·자유 댓글·DM·실시간 피드, 개인 랭킹, 교회 소속·교회 단위 집계·교회 랭킹 |
...
| S2' 방향 선택 **완료** | A 아침 햇살 단독 채택, 탭 명칭·앱 이름·보상 정책 확정 (`DEC-PWA-015·016·017·019`) | 방향 1개 확정 (2026-09-14) | 2026-09-14 |
| S3 디자인 시스템 | 채택안 토큰·컴포넌트·권위 층·접근성 상태를 `docs/specs/web/`에 정의 | 디자인·접근성 승인 | 2026-09-21 |
| S4 구현 계획 **완료** | ~~아키텍처·API·도메인(미션·정성·챌린지·관계·설교)·작업 분해·S5~S15 runbook~~ 2026-09-16 범위 축소: [PLAN-HD-001](../plans/active/2026-09-17-hoondok-mvp.md) — 3테이블·5 API·화면 4·Phase 1~4 | 구현 착수 승인 (2026-09-16) | 2026-09-16 |
...
| `DEC-PWA-015` | 2026-09-14 | **확정** | 5탭 채택과 명칭: 오늘 훈독 · AI 질문 · 말씀 · 가정예배 · **나의 정원**. 대안 "나의 뜰"·"나의 훈독"·"내 정성"은 미채택 |
| `DEC-PWA-016` | 2026-09-14 | **확정** | 앱 이름 **"훈독"**. 가안 표기를 해제한다. FFWPU 공식 승인 절차는 `DEC-PWA-001` 외부 게이트로 남는다 |
| `DEC-PWA-017` | 2026-09-14 | **확정** | 디자인 방향 **A 아침 햇살 단독**(라이트 `#fbfaf8`·감귤 `#c24721`·Pretendard 단일·16px·사진 히어로). B 저녁 등불 미채택. A+B 흡수 추천 3항목(시간대별 홈 전환·세리프 말씀 인용·B 팔레트 다크 모드)은 모두 도입하지 않는다. **베타는 라이트 고정**이며 다크 모드는 출시 후 수요를 보고 재검토한다 |
| `DEC-PWA-018` | 2026-09-10 | `[제안]` | 베타 기간 유료 잠금 없음. 수익화는 비용·사용 빈도 측정 뒤 |
| `DEC-PWA-019` | 2026-09-14 | **확정** | 개인 랭킹·순위와 달란트형 화폐·보상 배지 **모두 제외**. 진행률·참여 인원만 표시한다 |
...
| `DEC-PWA-022` | 2026-09-16 | **확정** | MVP 착수 결정 12개를 기본값으로 승인 — `/hoondok/*` + scope `/hoondok/`, 단일 루트 스코프 래퍼, 이메일+비밀번호(재설정 운영자 수동), 편성 운영자 수기, `featured_malssum` 시드 한정, lucide 치환, S4 범위 축소, KST 고정, 플래그 `NEXT_PUBLIC_HOONDOK_ENABLED`, 히어로 텍스트 카드. 약관·법적 주체만 `[확인 필요]`. 상세 [PLAN-HD-001](../plans/active/2026-09-17-hoondok-mvp.md) §1 |
| `DEC-PWA-023` | 2026-09-23 | **확정** | "함께 읽는 사람들" 8건 — ① 1단계 익명 숫자 → 2단계 초대 모임 ② 모임 안 완료자만 표시, 미완료자·독촉·순위·점수·보상 없음 ③ 나눔 = 한 줄 100자 + 반응 "함께 머물렀어요" 1종(훈독회 '대화') ④ 공식 정성·오늘 범위는 협회 공지를 편성자가 등록 ⑤ 첫 모임 = 소그룹(훈독가정교회), 리더가 초대 코드/카톡 링크로, 가족 모임은 후속 ⑥ 표시 이름은 모임별 ⑦ 정성 = 공식 + 모임 정성, "N일차", 빠진 날 비처벌 ⑧ 교회 소속·교회 비교·교회 랭킹 없음. **대체:** `AC-020-03` 응원 → `AC-024-04`, `AC-020-01` 교회 단위 삭제, `SCR-PWA-001` 교회 선택 삭제, F1·§2.2 "우리 교회 N명" → 익명 숫자, `AC-022-01` 기본값 "오늘 완료 여부" → "오늘 읽은 날만", `AC-023-02` "밀린 날" → "N일차". 상세 F8 · [벤치마크](../research/2026-09-23-hoondok-together-benchmark.md) |

---

## 13. S2' 진입 조건

- [x] 사용자가 §1 문제 정의, §3 5탭, §5 기능 7종, §6 보상 정책, §9 KPI를 검토했다.
- [x] 사용자가 디자인 방향을 골랐다 — **A 아침 햇살 단독** (2026-09-14, `DEC-PWA-017`).
- [x] `DEC-PWA-015·016·019`는 2026-09-14 확정했다. `DEC-PWA-020·021`(가행국 명칭·설교 운영 주체)은 외부 확인이 필요해 다음 세션으로 미뤘다.
...
**PRD 상태: 2026-09-16 승인** — 구현 계획 [PLAN-HD-001](../plans/active/2026-09-17-hoondok-mvp.md)(결정 12개 기본값 확정, S4 범위 축소: 3테이블·5 API·화면 4). F2 본문 등 모순은 2026-09-16 정정 주석으로 개정했다.

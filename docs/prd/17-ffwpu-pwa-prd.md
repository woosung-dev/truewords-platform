...
| `SCR-PWA-019` | 모임 만들기 | 오늘 | 소그룹 모임·이름(별칭)·오늘 범위 방식·모임 정성 → 초대 코드·카카오톡 공유 | 로그인 |
| `SCR-PWA-020` | 한 줄 나눔 쓰기 | 오늘 | 100자 한 줄, 모임에만 보임 | 로그인·모임 멤버 |

프로토타입 [`hoondok-ds/app.html`](prototypes/hoondok-ds/app.html)이 16화면을 모두 담는다. 화면 ↔ `data-screen` 대응은 그 파일 상단 목차 주석에 있다.
...
- `AC-024-05`: 오늘 범위는 공식 편성(협회 공지를 편성자가 등록)을 따르거나 리더가 원문 단락 범위를 지정한다. 정성은 공식 정성 + 모임 정성이며 "N일차 / 전체" 로 모임 전체 진행만 보인다. 빠진 날은 세지 않는다.

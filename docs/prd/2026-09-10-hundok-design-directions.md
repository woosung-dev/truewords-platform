...
- 작성일: 2026-09-10
- 상태: **A 아침 햇살 단독 채택 확정** (2026-09-14, `DEC-PWA-017`). B 저녁 등불 미채택
...
| 액센트 1색 | `#c24721` 감귤 (흰 글자 4.98:1) | `#e6b453` 따뜻한 금 |
...
> **2026-09-14 확정 (`DEC-PWA-017`): A 아침 햇살 단독 채택.**
> 아래 A+B 흡수 추천 4항목은 **미채택**이며 제안 시점의 기록으로만 남긴다.
> 시간대별 홈 전환(2번), 세리프 말씀 인용(3번), B 팔레트 다크 모드(4번)는 도입하지 않는다.
> 베타는 **라이트 고정**이고, 다크 모드는 출시 후 수요를 보고 재검토한다.

**A를 골격으로 채택하고, B의 두 요소를 흡수한다.** `[미채택]`
...
| 14 | 나의 정원 | 014 | 프로필·대표 말씀, 연속일 3종, 9월 달력(쉼 포함), 정성, 가족, 친구 |
...
- 프로토타입 HTML 과 스크린샷 38장은 2026-09-10 산출물이라 5번째 탭이 **"나의 뜰"**로 표기돼 있다. 2026-09-14 확정 명칭은 **"나의 정원"**(`DEC-PWA-015`)이다. 구현은 확정 명칭을 따르며, 시안은 결정 근거 기록이므로 재생성하지 않는다.

## 9. 다음 단계

1. ~~사용자가 A / B / A+B 흡수안(§4) 중 하나를 고른다~~ → **2026-09-14 A 단독 채택** (`DEC-PWA-017`).
2. ~~탭 명칭·보상 정책에 답한다~~ → **2026-09-14 확정** (`DEC-PWA-015·016·019`). 가행국 명칭·설교 운영 주체(`DEC-PWA-020·021`)는 외부 확인이 필요해 남는다.
3. S3 디자인 시스템: A 안의 토큰·컴포넌트(미션 카드·말씀 카드·권위 층·순서지·챌린지·달력·시트)·큰 글씨·WCAG 2.2 AA 상태를 `docs/specs/web/`에 정의한다. **라이트 단일 테마**로 정의하며 다크 팔레트는 정의하지 않는다.
...
| 2026-09-10 | PRD v2 + 2안 목업을 한 PR로 검토 요청 | 검토 완료 |
| 2026-09-14 | 방향 선택 — **A 아침 햇살 단독**. B 미채택, 흡수 요소 없음 (`DEC-PWA-017`) | 확정 |
| 2026-09-14 | 5번째 탭 명칭 **"나의 정원"** (`DEC-PWA-015`), 앱 이름 **"훈독"** 가안 해제 (`DEC-PWA-016`) | 확정 |
| 2026-09-14 | 개인 랭킹·달란트형 보상 **모두 제외** (`DEC-PWA-019`) | 확정 |
| 2026-09-14 | 베타는 **라이트 고정**. 다크 모드는 출시 후 수요 확인 뒤 재검토 | 확정 |

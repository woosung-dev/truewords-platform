"""RerankStage — Reranker factory 분기 + 점수 분포 로깅. intent 별 top_k 분기."""
...
class RerankStage:
    async def execute(self, ctx: ChatContext) -> ChatContext:
        ...
        if ctx.runtime_config and ctx.runtime_config.retrieval.rerank_enabled and ctx.results:
            ...
            # PR 7 측정 시 model 별 latency + score 분포 비교 데이터원.
            ...
            if rerank_scores:
                logger.info(
                    "rerank_score_dist",
                    extra={
                        "reranker_model": ctx.runtime_config.retrieval.reranker_model,

...
필터 적용:
    - volume: 100% 채워진 payload + .zfill(3) 일관 포맷 → 즉시 적용 (MatchValue exact)
...
def build_metadata_filter_conditions(meta: dict[str, int]) -> list[dict]:
    ...
    ``"001권"``, ``"056권"``, ``"123권"`` 같은 zero-pad 일관 포맷이므로
    ``MatchValue`` exact match 로 거짓 양성("5권" 입력 시 005/015/025... 매칭) 차단.
    ...
    if vol := meta.get("volume_num"):
        conditions.append(field_match("volume", f"{str(vol).zfill(3)}권"))

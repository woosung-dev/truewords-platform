...
audit P0-7 (2026-05-15): 기존 `src/qdrant_client.py` (파일) + `src/qdrant/`
(패키지) 동명 공존을 해소. 팩토리 + DEPRECATED 호환 헬퍼는 본 모듈 (`qdrant/
factory.py`), startup 헬퍼는 `qdrant/startup.py`. `qdrant/__init__.py` 가 모두
re-export 하므로 호출처는 `from src.qdrant import ...` 로 일관.

`src/qdrant_client.py` 는 backward-compat shim 으로 잔존 (tests 의 patch target,
backend/scripts 호환). 신규 코드는 `src.qdrant` 사용.

chat 핫패스 / admin / pipeline 모두 `get_raw_client()` (raw httpx, HTTP/1.1) 사용.
SDK `AsyncQdrantClient` / `QdrantClient` 는 backend/scripts 마이그레이션·스키마
작업 호환을 위해 잔존하지만 backend/src 내부에서는 사용하지 않는다.

...
const origins = [
  process.env.SMOKE_WEB_URL ?? "http://127.0.0.1:13000",
  process.env.SMOKE_ADMIN_URL ?? "http://127.0.0.1:13001",
];
...
for (const origin of origins) {
  ...
  assert.deepEqual(
    await (await fetch(`${origin}/api/chatbots`)).json(),
    await (await fetch(`${origin}/api/backend/chatbots`)).json(),
  );
  ...
  const denied = await fetch(`${origin}/api/backend/admin/users`, {
    method: "POST",
    headers: { cookie, "Content-Type": "application/json" },
    body: "{}",
  });
  assert.equal(denied.status, 403);
  const validated = await fetch(`${origin}/api/backend/admin/users`, {
    method: "POST",
  ...
  assert.equal(
    (
      await fetch(`${origin}/api/backend/admin/auth/logout`, {
        method: "POST",
...
const streamRequest = {
  method: "POST",
  headers: { "Content-Type": "application/json", "X-Requested-With": "XMLHttpRequest" },
  body: JSON.stringify({ chatbot_id: "all", query: "프록시 스트림 검증" }),
};
...
const cancellable = await fetch(`${origins[0]}/api/backend/chat/stream`, {
  ...streamRequest,

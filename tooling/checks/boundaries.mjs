...
export function checkImport(file, specifier) {
  ...
  if (/^@truewords\/ui-web(\/|$)/.test(target) || /^packages\/ui-web(?:\/|$)/.test(target)) return "UI와 테마는 각 앱이 소유합니다";
  ...
  if (file.startsWith("packages/api-client-ts/") && /^(react|react-dom|next)(\/|$)/.test(target)) return "API SDK는 플랫폼 중립이어야 합니다";
  return null;
}
export function checkSource(file, source) {
  return [...source.matchAll(/(?:\bfrom\s*|\bimport\s*\(?\s*|\brequire\s*\(\s*|@(?:import|source)\s*(?:url\(\s*)?)["']([^"']+)["']/g)].flatMap((match) => {

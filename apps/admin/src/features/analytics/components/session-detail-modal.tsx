...
import { Bookmark, ThumbsDown, ThumbsUp, X } from "lucide-react";
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm ${
          isUser ? "bg-foreground text-background" : "bg-admin-muted text-foreground"
        }`}
      >
        <div className={`text-[10px] mb-1 ${isUser ? "text-background/60" : "text-muted-foreground"}`}>
        ...
        {!isUser && msg.feedback && (
          <div className="mt-2 rounded-md border border-destructive/30 bg-destructive/10 px-2 py-1.5 text-[11px]">
            <div className="flex items-center gap-1.5">
              <Badge variant={normalize(msg.feedback.feedback_type) === "inaccurate" ? "destructive" : "secondary"}>
                {FEEDBACK_LABELS[normalize(msg.feedback.feedback_type)] ?? msg.feedback.feedback_type}
              </Badge>
              <span className="text-muted-foreground">{formatDateTime(msg.feedback.created_at)}</span>
            </div>
            {msg.feedback.comment && <div className="mt-1 text-foreground">“{msg.feedback.comment}”</div>}
          </div>
        )}
        ...
        {!isUser && (msg.citations ?? []).length > 0 && (
          <details className="mt-2 text-[11px]">
            ...
            <ul className="mt-1.5 space-y-1.5">
              {(msg.citations ?? []).map((c) => (
                <li key={c.rank_position} className="rounded bg-background/40 px-2 py-1">
                  <span className="font-medium">
                    [{c.source}] vol.{c.volume}
                  </span>{" "}
                  <span className="text-muted-foreground">score {c.relevance_score.toFixed(2)}</span>
                  <div className="mt-0.5 text-muted-foreground line-clamp-2">{c.text_snippet}</div>
                </li>
              ))}
            </ul>
          </details>
...
export default function SessionDetailModal({ open, onOpenChange, sessionId }: Props) {

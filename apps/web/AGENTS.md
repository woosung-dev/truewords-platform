...
- `/hoondok/*`(훈독)은 `NEXT_PUBLIC_HOONDOK_ENABLED=1` 일 때만 존재하고 AuthGuard 를 쓰지 않는다. 토큰·컴포넌트는 `src/app/hoondok.css`(`[data-app="hoondok"]` 스코프)·`src/components/hoondok`·`src/features/hoondok` 이 소유하며 `globals.css`·`:root`·`/design-system` 을 바꾸지 않는다. 검사는 `pnpm hoondok:check`, 계획은 [PLAN-HD-001](../../docs/plans/active/2026-09-17-hoondok-mvp.md).

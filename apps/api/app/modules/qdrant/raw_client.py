...
호출처:
- ``apps/api/app/modules/search/hybrid.py`` 등 검색 모듈 (PR-B 이후)
- ``apps/api/app/modules/cache/service.py`` (이미 inline 작성, 향후 일원화 가능)
- ``apps/api/app/modules/datasource/qdrant_service.py`` admin (PR-D 이후)

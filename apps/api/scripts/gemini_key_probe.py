...
(`app/modules/chat/service.py` Stage 순서가 Embedding → CacheCheck). 임베딩만 죽어도
...
(`app/core/common/gemini.py`) 가 보내지 않는 필드다. gemini-3.5 계열은
...
사용:
    ...
    cd apps/api && uv run python scripts/gemini_key_probe.py
...
# scripts/ 에서 app/* import 가능하도록 apps/api/ 를 sys.path 에 추가. **필수다.**
...
# app/core/common/gemini.py 의 embed_dense_query output_dimensionality 와 같아야 한다.
...
def _hint_one(bad: Probe) -> str:
    ...
    if bad.code.startswith("404"):
        return f"{bad.name} 모델 ID 가 사라졌다 — app/core/common/gemini.py 의 MODEL_* 갱신 필요"

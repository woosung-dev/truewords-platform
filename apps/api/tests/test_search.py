...
async def test_hybrid_search_returns_search_results():
    ...
    with (
        patch("app.modules.search.hybrid.embed_dense_query", new_callable=AsyncMock, return_value=[0.1] * 768),
        patch("app.modules.search.hybrid.embed_sparse_async", new_callable=AsyncMock, return_value=([1, 2], [0.5, 0.3])),
    ):
...
async def test_hybrid_search_without_filter_passes_none():
    ...
    with (
        patch("app.modules.search.hybrid.embed_dense_query", new_callable=AsyncMock, return_value=[0.0] * 768),
        patch("app.modules.search.hybrid.embed_sparse_async", new_callable=AsyncMock, return_value=([0], [1.0])),
    ):
...
async def test_hybrid_search_uses_rrf_fusion_with_dense_and_sparse_prefetch():
    ...
    with (
        patch("app.modules.search.hybrid.embed_dense_query", new_callable=AsyncMock, return_value=[0.1] * 4),
        patch(
            "app.modules.search.hybrid.embed_sparse_async",

...
from app.modules.chat.models import ResearchSession
from app.modules.chat.pipeline.context import ChatContext
from app.modules.chat.pipeline.state import PipelineState, force_transition_to
...
class TestForceTransitionTo:
    def test_force_transition_updates_state_and_logs_warning(
        ...
        with caplog.at_level(logging.WARNING, logger="app.modules.chat.pipeline.state"):
            force_transition_to(
...
class TestStreamAbortIntegration:
    ...
    async def test_stream_abort_force_transitions_to_STREAM_ABORTED(
        ...
        with (
            ...
            patch(
                "app.modules.chat.service.generate_answer_stream",
                side_effect=cancelled_gen,
            ),
            patch(
                "app.modules.chat.pipeline.stages.search.cascading_search",
                new_callable=AsyncMock,
                return_value=[],
            ),
            patch(
                "app.modules.chat.pipeline.stages.search.fallback_search",
                new_callable=AsyncMock,
                return_value=([], "none"),
            ),
            patch(
                "app.modules.qdrant.factory.get_async_client",
                return_value=MagicMock(),
            ),
            caplog.at_level(logging.WARNING, logger="app.modules.chat.pipeline.state"),
        ):

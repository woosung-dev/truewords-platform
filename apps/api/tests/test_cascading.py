...
from app.modules.search.cascading import cascading_search, SearchTier, CascadingConfig
from app.modules.search.hybrid import SearchResult

# cascading_search가 임베딩을 직접 계산하므로 모든 테스트에서 mock 필요
_DENSE_PATCH = "app.modules.search.cascading.embed_dense_query"
_SPARSE_PATCH = "app.modules.search.cascading.embed_sparse_async"
...
async def test_cascading_returns_first_tier_when_sufficient():
    ...
    with (
        patch("app.modules.search.cascading.hybrid_search", new_callable=AsyncMock) as mock_search,
...
from app.modules.search.exceptions import SearchFailedError  # noqa: E402
...
async def test_cascading_search_tier_fallback_on_first_tier_failure():
    ...
    with patch("app.modules.search.cascading.hybrid_search", side_effect=fake_hybrid_search):
        with patch(
            "app.modules.search.cascading.embed_dense_query",
...
async def test_cascading_search_raises_search_failed_when_all_tiers_fail():
    ...
    with patch("app.modules.search.cascading.hybrid_search", side_effect=always_fail):
        with patch(
            "app.modules.search.cascading.embed_dense_query",
            new_callable=AsyncMock,
            return_value=[0.1] * 3072,
...
async def test_cascading_logs_score_distribution(caplog):
    ...
    with (
        ...
        patch(_DENSE_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
        patch(_SPARSE_PATCH, new_callable=AsyncMock, return_value=([1], [0.5])),
        caplog.at_level(logging.INFO, logger="app.modules.search.cascading"),
    ):
        mock_search.return_value = a_results

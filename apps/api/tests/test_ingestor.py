...
from app.modules.pipeline.chunker import Chunk
from app.modules.pipeline.ingestor import ingest_chunks
...
def _patch_io(upsert_calls: list):
    ...
    return patch("app.modules.pipeline.ingestor._sync_upsert", side_effect=fake_upsert)
...
def test_ingest_calls_upsert_with_correct_payload():
    ...
    with (
        ...
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536] * 2),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1, 2], [0.5, 0.3])] * 2),
    ):
...
def test_ingest_payload_contains_text_and_volume():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.0] * 1536]),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([0], [1.0])]),
    ):
        ingest_chunks("test_collection", chunks)
...
def test_ingest_payload_includes_source():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536]),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1, 2], [0.5, 0.3])]),
    ):
        ingest_chunks("test_collection", chunks)
...
def test_ingest_payload_includes_title_and_date():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536]),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1], [0.5])]),
    ):
        ingest_chunks("test_collection", chunks)
...
def test_ingest_resumes_from_start_chunk():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536] * 3),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1], [0.5])] * 3),
    ):
...
def test_ingest_passes_title_to_embed_dense_batch():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536]) as mock_embed,
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1], [0.5])]),
    ):
...
def test_payload_sources_overrides_chunk_source():
    ...
    with (
        _patch_io(upsert_calls),
        patch("app.modules.pipeline.ingestor.embed_dense_batch", return_value=[[0.1] * 1536] * 2),
        patch("app.modules.pipeline.ingestor.embed_sparse_batch", return_value=[([1], [0.5])] * 2),
    ):
        ingest_chunks(
            "test_collection",
            chunks,

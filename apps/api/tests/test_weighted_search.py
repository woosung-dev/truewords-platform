...
def _mock_embeddings():
    with (
        patch("app.modules.search.weighted.embed_dense_query", new_callable=AsyncMock, return_value=MOCK_DENSE),
        patch("app.modules.search.weighted.embed_sparse_async", new_callable=AsyncMock, return_value=MOCK_SPARSE),
    ):
        yield
...
async def test_basic_3_sources():
    ...
    from app.modules.search.weighted import weighted_search, WeightedConfig, WeightedSource
...
async def test_empty_config():
    """소스 없음 → 빈 리스트."""
    from app.modules.search.weighted import weighted_search, WeightedConfig
...
async def test_weighted_logs_score_distribution(caplog):
    ...
    with (
        patch("app.modules.search.weighted.hybrid_search", side_effect=fake_hybrid),
        caplog.at_level(logging.INFO, logger="app.modules.search.weighted"),
    ):

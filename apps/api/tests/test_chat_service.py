...
_EMBED_PATCH = "app.modules.chat.pipeline.stages.embedding.embed_dense_query"
...
async def test_process_chat_without_rerank():
    ...
    with (
        patch("app.modules.qdrant.factory.get_async_client") as mock_qdrant,
        patch("app.modules.chat.pipeline.stages.intent_classifier.classify_intent", new_callable=AsyncMock, return_value="conceptual"),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=results) as mock_cascade,
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="답변입니다."),
        patch("app.modules.chat.pipeline.stages.rerank.rerank", new_callable=AsyncMock) as mock_rerank,
...
async def test_process_chat_with_rerank():
    ...
    with (
        ...
        patch("app.modules.chat.pipeline.stages.intent_classifier.classify_intent", new_callable=AsyncMock, return_value="conceptual"),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=results),
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="재순위 답변."),
        patch("app.modules.chat.pipeline.stages.rerank.rerank", new_callable=AsyncMock, return_value=reranked_results) as mock_rerank,
        patch(_EMBED_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
    ):
        response = await service.process_chat(ChatRequest(query="질문"))
...
async def test_process_chat_records_rerank_in_search_event():
    ...
    with (
        ...
        patch("app.modules.chat.pipeline.stages.intent_classifier.classify_intent", new_callable=AsyncMock, return_value="conceptual"),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=results),
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="답변"),
        patch("app.modules.chat.pipeline.stages.rerank.rerank", new_callable=AsyncMock, return_value=reranked),
        patch(_EMBED_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
    ):
...
async def test_process_chat_single_commit():
    ...
    with (
        patch("app.modules.qdrant.factory.get_async_client"),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=_make_search_results(3)),
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="답변"),
        patch(_EMBED_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
    ):
        await service.process_chat(ChatRequest(query="질문"))
...
async def test_process_chat_empty_results():
    ...
    with (
        patch("app.modules.qdrant.factory.get_async_client"),
        patch("app.modules.chat.pipeline.stages.intent_classifier.classify_intent", new_callable=AsyncMock, return_value="conceptual"),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=[]),
        patch("app.modules.chat.pipeline.stages.search.fallback_search", new_callable=AsyncMock, return_value=([], "suggestions")),
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="해당 내용을 말씀에서 찾지 못했습니다."),
        patch(_EMBED_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
    ):
...
async def test_process_chat_default_intent_uses_conceptual_slice():
    ...
    with (
        patch("app.modules.qdrant.factory.get_async_client"),
        patch(
            "app.modules.chat.pipeline.stages.intent_classifier.classify_intent",
            new_callable=AsyncMock,
            return_value="conceptual",
        ),
        patch("app.modules.chat.pipeline.stages.search.cascading_search", new_callable=AsyncMock, return_value=results),
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock, return_value="답변") as mock_gen,
        patch(_EMBED_PATCH, new_callable=AsyncMock, return_value=[0.1] * 3072),
    ):
        await service.process_chat(ChatRequest(query="질문"))
...
async def test_process_chat_wraps_embedding_failure_as_embedding_failed_error():
    ...
    with (
        ...
        patch("app.modules.chat.pipeline.stages.generation.generate_answer", new_callable=AsyncMock),

...
def _make(session, tmp_path, *, api_key: str | None = "test-key", limit: int = 900_000, synth=None):
    qdrant = AsyncMock()
...
async def test_cache_hit_does_not_synthesize_again(db, tmp_path):
    ...
    first = await service.chunk_audio(CHUNK_ID, "sulafat")
    second = await service.chunk_audio(CHUNK_ID, "sulafat")
...
async def test_other_voice_is_separate_cache_entry(db, tmp_path):
    await _allow(db, scope_full_text=True)
    service, _, synth = _make(db, tmp_path)
    await service.chunk_audio(CHUNK_ID, "sulafat")
    await service.chunk_audio(CHUNK_ID, "iapetus")
...
async def test_concurrent_same_key_synthesizes_once(db, tmp_path):
    ...
    results = await asyncio.gather(*(service.chunk_audio(CHUNK_ID, "aoede") for _ in range(3)))
...
async def test_monthly_limit_blocks_new_synthesis_but_serves_cache(db, tmp_path):
    ...
    await service.chunk_audio(CHUNK_ID, "sulafat")  # 상한 안 — 만든다

    # 다른 목소리는 새 합성이라 상한을 넘는다 → 429, Google 을 부르지 않는다.
    with pytest.raises(TtsError) as exc:
        await service.chunk_audio(CHUNK_ID, "aoede")
    assert (exc.value.status_code, exc.value.error_code) == (429, "TTS_QUOTA_EXCEEDED")
    assert synth.await_count == 1
    # 이미 만든 파일은 상한과 무관하게 준다.
    assert (await service.chunk_audio(CHUNK_ID, "sulafat")).is_cached
...
async def test_invalid_voice_is_rejected_before_lookup(db, tmp_path):
    ...
    with pytest.raises(TtsError) as exc:
        await service.chunk_audio(CHUNK_ID, "ko-KR-Chirp3-HD-Sulafat")
...
async def test_chunk_text_trims_overlap_like_words_page(db, tmp_path):
    ...
    await service.chunk_audio("3", "sulafat")
...
@pytest.mark.asyncio
async def test_upstream_failure_maps_to_502_and_records_nothing(db, tmp_path):
    await _allow(db, scope_full_text=True)

...
async def test_get_cache_service_lazy_init_on_first_call():
    ...
    with patch("app.modules.chat.dependencies.ensure_cache_collection") as mock_ensure:
...
async def test_get_cache_service_retries_after_failure_cooldown():
    ...
    with (
        patch("app.modules.chat.dependencies._cache_last_failure_monotonic", 0.0),
        patch("app.modules.chat.dependencies.time.monotonic", return_value=300.0),
        patch("app.modules.chat.dependencies.ensure_cache_collection") as mock_ensure,
    ):
        result = await get_cache_service(mock_request)
...
async def test_get_cache_service_skips_retry_before_cooldown():
    ...
    with (
        patch("app.modules.chat.dependencies._cache_last_failure_monotonic", 0.0),
        patch("app.modules.chat.dependencies.time.monotonic", return_value=299.0),
        patch("app.modules.chat.dependencies.ensure_cache_collection") as mock_ensure,
    ):
        result = await get_cache_service(mock_request)
...
async def test_get_cache_service_lazy_init_only_once_under_concurrency():
    ...
    with patch("app.modules.chat.dependencies.ensure_cache_collection", side_effect=slow_ensure):
        # 동시 5건 요청

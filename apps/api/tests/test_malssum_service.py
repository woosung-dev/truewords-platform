...
import app.modules.malssum.service as malssum
...
def test_chat_response_coerces_featured_malssum():
    ...
    """
    from app.modules.chat.schemas import ChatResponse, FeaturedMalssum
...
async def test_pick_for_answer_empty_pool_no_llm(monkeypatch):
    ...
    called = AsyncMock()
    monkeypatch.setattr("app.core.common.gemini.generate_text", called)
...
async def test_pick_for_answer_matches_classified_theme(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.core.common.gemini.generate_text", AsyncMock(return_value="위로")
    )
...
async def test_pick_for_answer_unmatched_falls_back_to_whole_pool(monkeypatch):
    ...
    monkeypatch.setattr(malssum, "_items", items)
    monkeypatch.setattr(
        "app.core.common.gemini.generate_text", AsyncMock(return_value="존재하지않는주제")
    )
...
async def test_pick_for_answer_llm_failure_falls_back(monkeypatch):
    ...
    monkeypatch.setattr(malssum, "_items", items)
    monkeypatch.setattr(
        "app.core.common.gemini.generate_text", AsyncMock(side_effect=RuntimeError("LLM down"))
    )

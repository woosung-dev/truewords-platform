...
test("정규 API 프록시와 구 alias가 같은 응답을 제공한다", async ({ request }) => {
  ...
  const canonicalReaction = await request.get(
    `${webOrigin}/api/backend/api/chat/messages/00000000-0000-4000-8000-000000000000/reactions`,
  );
  const legacyReaction = await request.get(
    `${webOrigin}/api/chat/messages/00000000-0000-4000-8000-000000000000/reactions`,
  );
...
test("같은 API를 써도 다른 계정의 실제 대화 기록은 열 수 없다", async ({ playwright }) => {
  ...
  try {
    expect(
      (
        await owner.post("/api/backend/admin/auth/login", {
          headers: csrf,
          data: { email: adminEmail, password: "test1234" },
        })
      ).status(),
    ...
    expect(
      (
        await other.post("/api/backend/admin/auth/login", {
          headers: csrf,
    ...
    const created = await owner.post("/api/backend/chat", {
      headers: csrf,
      data: { query: "대화 소유권 검증", chatbot_id: bots[0].chatbot_id },
    });

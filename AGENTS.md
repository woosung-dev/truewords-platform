...
- 운영은 Oracle ARM VM 6컨테이너(web·admin·backend·Qdrant·Postgres·Cloudflare Tunnel)다. **2026-09-06 web/admin 분리 컷오버 완료**: `app.woosung.dev` → web, `truewords-admin.woosung.dev` → admin. 배포는 `make deploy-*`(deploy-guard, 프론트는 `--no-deps`)로만 하며 단계별 승인이 필요하다.
